...
def create_densenet121_model(
    input_shape, num_classes, trainable_layers=0, learning_rate=0.0001
):
    # Pre-trained model yükleniyor
    ...
    # Eğitimde kullanılacak katmanların belirlenmesi
...
# & _________________________________________ Eğitim İşlemi ________________________________________________
# * 10-) --------------------------------------------------------------------------------------------------------
# iki aşamalı eğitim sürecini tek bir fonksiyonla daha modüler hale getirebiliriz.


def train_model(
    model, train_images, train_labels, valid_images, valid_labels, epochs, callbacks
):
    history = model.fit(
        train_images,
        train_labels,
...
def create_callbacks(
    ...
    - prefix (str): Checkpoint dosya adı için başa eklenecek string ifade.
    ...
    ckp_path = os.path.join(output_directory, prefix + checkpoint_name)
...
model = create_densenet121_model(
    image_shape, num_classes, trainable_layers=0, learning_rate=0.0001
)
...
callbacks = create_callbacks(output_directory, train_df.shape[0], prefix="pretrain_")
...
model = create_densenet121_model(
    image_shape, num_classes, trainable_layers=30, learning_rate=0.00001
)

# ~ Callback'lerin oluşturulması
callbacks = create_callbacks(output_directory, train_df.shape[0], prefix="fineTunning_")
...
def save_history(history, output_directory, filename):
    """
    Eğitim geçmişini (history) belirtilen dizine kaydeder.

    Parameters:
    - history (History): Keras History nesnesi.
    - output_directory (str): Dosyanın kaydedileceği dizin.
    - filename (str): Kaydedilecek dosyanın adı.

    Returns:
    - None
    """
    history_file_path = os.path.join(output_directory, filename)
    with open(history_file_path, "wb") as f:
        np.save(f, history.history)
    print(f"History saved to --> {history_file_path}")

# Fonksiyonun kullanım örnekleri:
save_history(history_pretrain, output_directory, "history_DenseNet121_pretrain.npy")
save_history(history_finetune, output_directory, "history_DenseNet121_fineTunning.npy")
